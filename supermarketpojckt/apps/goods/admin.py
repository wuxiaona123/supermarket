...
class RotationModelAdmin(admin.ModelAdmin):
    ...
    # 自定义显示列，在models类中对字段有方法的就填方法名
    list_display = ['id', 'bannerName', 'goodsSKU', 'goodsImg', 'bannerOrder', 'add_time', 'up_time', 'is_delete']

    # 设置在列表页字段上添加一个 a标签, 从而进入到编辑页面,在models类中对字段有方法的就填方法名
    list_display_links = ['id', 'bannerName', 'goodsSKU', 'goodsImg', 'bannerOrder', 'add_time', 'up_time', 'is_delete']
...
class UnitModelAdmin(admin.ModelAdmin):
    ...
    # 自定义显示列，在models类中对字段有方法的就填方法名
    list_display = ['id', 'unitname', 'add_time', 'up_time', 'is_delete']

    # 设置在列表页字段上添加一个 a标签, 从而进入到编辑页面,在models类中对字段有方法的就填方法名
    list_display_links = ['id', 'unitname', 'add_time', 'up_time', 'is_delete']
...
class GoodsClassModelAdmin(admin.ModelAdmin):
    ...
    # 自定义显示列，在models类中对字段有方法的就填方法名
    list_display = ['id', 'classname', 'classbrief', 'add_time', 'up_time', 'is_delete']

    # 设置在列表页字段上添加一个 a标签, 从而进入到编辑页面,在models类中对字段有方法的就填方法名
    list_display_links = ['id', 'classname', 'classbrief', 'add_time', 'up_time', 'is_delete']
...
class GoodsSPUModelAdmin(admin.ModelAdmin):
    ...
    # 自定义显示列，在models类中对字段有方法的就填方法名
    list_display = ['id', 'SPUname', 'SPUdetails']

    # 设置在列表页字段上添加一个 a标签, 从而进入到编辑页面,在models类中对字段有方法的就填方法名
    list_display_links = ['id', 'SPUname', 'SPUdetails']

    # 列表右侧栏过滤器,只能写一个
    list_filter = ['SPUname']

    # 搜索框,搜索字段 也只能写一个
    search_fields = ['SPUname']
